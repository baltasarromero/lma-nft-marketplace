...
describe("NFTMarketplace", function () {
	...
	async function bidAuctionsDataFixture(): Promise<MarketplaceDataForBids> {
		...
		await testCarsNFT
			.connect(nftAuctioneer)
			.approve(nftMarketplace.address, tokenId3);
	...
	describe("Auctions", function () {
		describe("Bids", function () {
			...
			this.beforeEach(async function () {
				marketplaceDataForBids = await loadFixture(
					bidAuctionsDataFixture
				);
			...
			it("Should allow to place a bid if there were no previous bids", async function () {
				...
				const auction =
					await marketplaceDataForBids.nftMarketplace.auctions(
						auction1.auctionKey
					);
			...
			it("Should not allow a bid lower than the current highest bid", async function () {
				...
				// Check that the bid wasn't placed
				const auction =
					await marketplaceDataForBids.nftMarketplace.auctions(
						auction1.auctionKey
					);
